...
fn main(){
    ...
    //     edge_list = stud_rust_base::io::read_binary_graph(Path::new(path), "travel_time").into();
    // } else if variant == "bgeo" {
    //     edge_list = stud_rust_base::io::read_binary_graph(Path::new(path), "geo_distance").into();
    // } else {
