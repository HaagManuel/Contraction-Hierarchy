...
pub fn benchmark_dijkstra(edge_list: EdgeList, _coord: Vec<(Coordinate, Coordinate)>) {
    let graph: AdjacencyList = edge_list.into();
    let mut dd1: DijkstraData = DijkstraData::new(graph.num_nodes());
    let mut dd2: DijkstraData = DijkstraData::new(graph.num_nodes());
    let mut dij = Dijkstra::new(&graph, &mut dd1);
    let mut dij2 = Dijkstra::new(&graph, &mut dd2);
    
    // let num = 5;
    let starts = vec![1];
    let start_targets: Vec<(NodeId, Vec<(NodeId, Distance)>)> = starts.iter().map(|&s| (s, dijkstra_rank_targets(&mut dij, s))).collect();
    ...
    for (s, targets) in &start_targets {
        ...
        for (t, d) in targets {
            print!("{d} ");
            let elapsed = measure_time(|| {dij.one_to_one(*s, *t, &ZeroPotential::new());});
            println!("{:?}", elapsed);
        }
        println!("");        
    }
    
    println!("bidir-dijkstra-alternating");
    for (s, targets) in &start_targets {
        println!("s {s}");
        for (t, d) in targets {
            print!("{d} ");
            let elapsed = measure_time(|| {Dijkstra::bidrektional_dijkstra::<Alternating, SumQueueKeyStop>(*s, *t, &mut dij, &mut dij2);});
            println!("{:?}", elapsed);
        }
        println!("");        
    }

    println!("bidir-dijkstra-smaller-pq");
    for (s, targets) in &start_targets {
        println!("s {s}");
        for (t, d) in targets {
            print!("{d} ");
            let elapsed = measure_time(|| {Dijkstra::bidrektional_dijkstra::<SmallerQueue, SumQueueKeyStop>(*s, *t, &mut dij, &mut dij2);}); 
            println!("{:?}", elapsed);
        }
        println!("");        
    }
    
    println!("bidir-dijkstra-smaller-pq-key");
    for (s, targets) in &start_targets {
        println!("s {s}");
        for (t, d) in targets {
            print!("{d} ");
            let elapsed = measure_time(|| {Dijkstra::bidrektional_dijkstra::<SmallerQueueKey, SumQueueKeyStop>(*s, *t, &mut dij, &mut dij2);}); 
            println!("{:?}", elapsed);
        }
        println!("");        
    }

    // println!("L2-potential");
    // let pot = L2Potential::new(&coord);
    // for (s, targets) in &start_targets {
    //     println!("s {s}");
    //     for (t, d) in targets {
    //         print!("{d} ");
    ...
    // println!("ALT-potential");
    // let landmarks = vec![11310, 5000, 32244, 3312];
    // let mut landmarks: Vec<NodeId> = Vec::new();
    // let num_marks = 16;
    // for i in 0..num_marks {
    //     landmarks.push(i * 500);
    // }
    // let pot = AltPotential::new(&mut dij, landmarks);
    // for (s, targets) in &start_targets {
    //     println!("s {s}");
    //     for (t, d) in targets {
    //         print!("{d} ");
